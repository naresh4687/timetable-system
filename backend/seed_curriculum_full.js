...
mongoose.connect(process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/timetable')
  .then(async () => {
    ...
    // WIPE all existing curriculum records
    const deleted = await Curriculum.deleteMany({});
    console.log(`Cleared ${deleted.deletedCount} old curriculum records`);

    // Re-insert all semesters fresh
    for (const semData of allSemesters) {
      await Curriculum.create(semData);
      console.log(`✅ Sem ${semData.semester}: ${semData.subjects.length} subjects`);
    }

    const all = await Curriculum.find({}, { semester: 1, _id: 0 }).sort({ semester: 1 });
    console.log('\n📚 DB now has semesters:', all.map(c => c.semester).join(', '));
    process.exit(0);
  })
