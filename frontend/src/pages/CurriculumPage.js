...
import { LuBookOpen, LuPlus, LuTrash2, LuSave, LuUpload } from 'react-icons/lu';
...
export default function CurriculumPage() {
  ...
  const handleSave = async () => {
    ...
    if (!valid) { toast.error('Please fill in all subject fields.'); return; }
    setSaving(true);
    const grouped = {};
    ...
    try {
      ...
      toast.success('All curriculum data saved!');
      await fetchAll();
    } catch (err) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (data.subjects && data.subjects.length > 0) {
        ...
        if (flattened.length > 0) {
          setSubjects(flattened);
          toast.success('Document parsed! Review and save.', { id: 'parse-toast', duration: 5000 });
        } else {
          toast.error('Could not map subjects to semesters.', { id: 'parse-toast' });
        }
      } else {
        toast.error('No subjects extracted.', { id: 'parse-toast' });
      }
    } catch (err) {
  ...
  if (loadingAll) return <div className="loading-wrap"><div className="spinner" /></div>;
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Define subjects, credits & hours per semester</p>
        </div>
      </div>
      ...
      <div className="card mb-2">
        ...
        <div className="grid-2">
          ...
          <div className="form-group">
            ...
            <input type="number" min={1} max={10} value={sections} onChange={(e) => setSections(Math.max(1, Number(e.target.value)))} />
      ...
      {/* Master Subject Library */}
      <div className="card mb-2">
        <h3 className="card-title mb-1">Subject Library (Master List)</h3>
        <p className="text-muted text-sm mb-2">Add reusable subjects here so they auto-fill in the curriculum table below.</p>
        <form onSubmit={handleCreateMaster} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,2fr) minmax(0,1fr) minmax(0,1fr) minmax(0,1fr) minmax(0,1fr) auto', gap: '0.5rem', alignItems: 'end' }}>
          ...
          <button type="submit" className="btn btn-secondary btn-sm" style={{ marginBottom: '0.1rem' }}><LuPlus size={14} /> Add</button>
        </form>
      </div>
      ...
      <div className="card mb-2">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', marginBottom: '1rem' }}>
          <h3 className="card-title" style={{ margin: 0 }}>All Semesters</h3>
          <div className="action-buttons">
            <div style={{ position: 'relative' }}>
              <input type="file" id="curriculumFile" accept=".pdf,.doc,.docx" style={{ display: 'none' }} onChange={handleFileUpload} disabled={parsing} />
              <label htmlFor="curriculumFile" className="btn btn-secondary btn-sm"
                style={{ cursor: parsing ? 'not-allowed' : 'pointer', opacity: parsing ? 0.7 : 1 }}>
                <LuUpload size={14} /> {parsing ? 'Parsing...' : 'Bulk Upload (PDF/DOC)'}
              </label>
            </div>
            <button className="btn btn-secondary btn-sm" onClick={addSubject}>
              <LuPlus size={14} /> Add Subject
            </button>
          </div>
        </div>
            ...
            <tbody>
              {subjects.map((sub, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <select value={sub.semester} onChange={(e) => updateSubject(idx, 'semester', Number(e.target.value))}
                      style={{ width: '100%', fontSize: '0.82rem', padding: '0.4rem' }}>
                      {[1,2,3,4,5,6,7,8].map(s => <option key={s} value={s}>Sem {s}</option>)}
                    </select>
                  </td>
                  <td>
                    <input list={`subject-names-${idx}`} value={sub.name}
                      onChange={(e) => {
                        const val = e.target.value;
                        const matched = masterSubjects.find(m => m.name === val);
                        if (matched) {
                          setSubjects(prev => { const next = [...prev]; next[idx] = { ...next[idx], name: matched.name, code: matched.code, type: matched.type, credits: matched.credits, hoursPerWeek: matched.hoursPerWeek }; return next; });
                        } else { updateSubject(idx, 'name', val); }
                      }}
                      placeholder="e.g. Data Structures" style={{ width: '100%', fontSize: '0.82rem', padding: '0.4rem' }} />
                    <datalist id={`subject-names-${idx}`}>
                      {masterSubjects.map(m => <option key={m._id || m.code} value={m.name} />)}
                    </datalist>
                  </td>
                  <td><input value={sub.code} onChange={(e) => updateSubject(idx, 'code', e.target.value)} placeholder="CS201" style={{ width: '100%', fontSize: '0.82rem', padding: '0.4rem' }} /></td>
                  <td>
                    <select value={sub.type} onChange={(e) => updateSubject(idx, 'type', e.target.value)} style={{ width: '100%', fontSize: '0.82rem', padding: '0.4rem' }}>
                      <option value="theory">Theory</option><option value="lab">Lab</option>
                    </select>
                  </td>
                  <td><input type="number" min={1} max={10} value={sub.credits} onChange={(e) => updateSubject(idx, 'credits', e.target.value)} style={{ width: '100%', fontSize: '0.82rem', padding: '0.4rem' }} /></td>
                  <td><input type="number" min={1} max={20} value={sub.hoursPerWeek} onChange={(e) => updateSubject(idx, 'hoursPerWeek', e.target.value)} style={{ width: '100%', fontSize: '0.82rem', padding: '0.4rem' }} /></td>
                  <td style={{ textAlign: 'center' }}>
                    {subjects.length > 1 && (
                      <button className="btn btn-danger btn-sm" style={{ padding: '0.2rem 0.5rem' }} onClick={() => removeSubject(idx)} title="Remove">
                        <LuTrash2 size={13} />
                      </button>
                    )}
                  </td>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--border)' }}>
          <div className="text-sm text-muted">
            Total Credits: <strong>{subjects.reduce((sum, s) => sum + (Number(s.credits) || 0), 0)}</strong> &nbsp;|&nbsp;
            Total Hrs/Week: <strong>{subjects.reduce((sum, s) => sum + (Number(s.hoursPerWeek) || 0), 0)}</strong>
          </div>
          <button className="btn btn-primary" onClick={handleSave} disabled={saving}>
            <LuSave size={16} /> {saving ? 'Saving...' : 'Save Curriculum'}
          </button>
        </div>
      </div>
      ...
      <div className="alert alert-info" style={{ marginBottom: 0 }}>
        <strong>Tip:</strong> After defining curriculum for all semesters, go to <strong>Create Timetable</strong> to bulk-generate timetables.
      </div>
    </div>
  );
}
