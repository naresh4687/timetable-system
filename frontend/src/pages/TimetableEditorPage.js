...
// ─── CREATE MODE: Simple bulk generation form ────────────────────────────────
function CreateTimetableForm() {
  ...
  const handleGenerate = async () => {
    ...
    if (readySemesters.length === 0) {
      toast.error('No curricula defined for selected semesters. Please set up curriculum first.');
      return;
    }
    ...
    try {
      ...
      if (data.missingSemesters?.length > 0) {
        msg += ` (Missing curriculum for Sem: ${data.missingSemesters.join(', ')})`;
      }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Auto-generate timetables for all years & sections from curriculum
          </p>
        </div>
      </div>
      ...
      <div className="card mb-2">
        ...
        <div className="grid-2">
          <div className="form-group" style={{ gridColumn: '1 / -1' }}>
            <label>Title *</label>
            <input
              placeholder="e.g. AD&DS 2025-2026 Odd Semester"
              value={title}
      ...
      {/* Preview: which semesters will be generated */}
      <div className="card mb-2">
        <h3 className="card-title mb-2">Timetables to Generate</h3>
        <p className="text-muted text-sm" style={{ marginBottom: '1rem' }}>
          Based on your curriculum definitions, the following timetables will be auto-created with subjects filled in.
          Labs will be allocated 4 consecutive periods.
        </p>
            ...
            return (
              <div
                ...
                style={{
                  padding: '0.8rem 1rem',
                  borderRadius: '10px',
                  background: curr ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)',
                  border: `1px solid ${curr ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.25)'}`,
                }}
              >
                <div style={{ fontWeight: 700, fontSize: '0.9rem', marginBottom: '0.3rem' }}>
                  {curr ? '✅' : '❌'} Year {year} — Semester {sem}
                </div>
                {curr ? (
                  <div className="text-sm text-muted">
                    ...
                    <br />
                    ↳ Will create <strong>{curr.sections}</strong> timetable(s)
                  </div>
                ) : (
                  <div className="text-sm" style={{ color: '#ef4444' }}>
                    No curriculum defined — <a href="/curriculum" style={{ color: '#60a5fa', textDecoration: 'underline' }}>Set up curriculum →</a>
                  </div>
                )}
              </div>
            );
        ...
        >
          ...
          {readySemesters.reduce((sum, sem) => {
            const c = allCurricula.find((cr) => cr.semester === sem);
            return sum + (c?.sections || 0);
          }, 0)}{' '}
          timetable(s) will be generated
        </div>
      </div>

      {/* Generate button */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
        <button className="btn btn-secondary" onClick={() => navigate('/timetables')}>
          Cancel
        </button>
        <button
          className="btn btn-primary"
          onClick={handleGenerate}
          disabled={loading || readySemesters.length === 0}
          style={{ minWidth: 200 }}
        >
          {loading ? '⏳ Generating...' : `🚀 Generate ${readySemesters.reduce((sum, sem) => {
            const c = allCurricula.find((cr) => cr.semester === sem);
            return sum + (c?.sections || 0);
          }, 0)} Timetable(s)`}
        </button>
...
// ─── EDIT MODE: Full schedule editor (used for /timetables/:id/edit) ─────────
function EditTimetableForm({ id }) {
  ...
  useEffect(() => {
    userAPI.getStaff().then(({ data }) => setStaff(data.staff)).catch(() => { });
  }, []);
    ...
    <div>
      ...
      <div className="card mb-2" style={{ padding: '0.75rem 1rem' }}>
        <div className="flex-gap text-sm">
          <span>🏛️ <strong>{meta.department}</strong></span>
          <span>·</span>
      ...
      <div className="card" style={{ padding: '1rem' }}>
        <h3 className="card-title mb-2">Weekly Schedule</h3>
...
// ─── Main component: routes to Create or Edit ────────────────────────────────
export default function TimetableEditorPage() {
  const { id } = useParams();

  if (id) {
    return <EditTimetableForm id={id} />;
  }
